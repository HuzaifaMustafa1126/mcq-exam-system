...
export const getDashboardMetrics = async () => {
  const [
    ...
    getCount("SELECT COUNT(*) AS total FROM exams WHERE status = ?", [
      "active",
    ]),
  ]);
...
export const getStudentDashboard = async (userId) => {
  ...
  if (!studentRows[0])
    throw new AppError("Student profile not found", HTTP_STATUS.FORBIDDEN);
  ...
  const [availableRows, upcomingRows, completedRows, averageRows, recentRows] =
    await Promise.all([
      pool.execute(
        `SELECT COUNT(*) AS total FROM exams WHERE ${availableWhere}`,
      ),
      pool.execute(
        `SELECT COUNT(*) AS total FROM exams WHERE exams.status IN ('published', 'active') AND exams.starts_at > NOW()`,
      ),
      pool.execute(
        `SELECT COUNT(*) AS total FROM results INNER JOIN student_exams ON student_exams.id = results.student_exam_id WHERE student_exams.student_id = ?`,
        [studentId],
      ),
      pool.execute(
        `SELECT COALESCE(ROUND(AVG(results.percentage), 2), 0) AS averageScore FROM results INNER JOIN student_exams ON student_exams.id = results.student_exam_id WHERE student_exams.student_id = ?`,
        [studentId],
      ),
      pool.execute(
        `SELECT results.id, student_exams.id AS attemptId, exams.title AS examTitle, subjects.name AS subject, results.percentage, UPPER(results.status) AS status, student_exams.submitted_at AS submittedAt
      ...
      WHERE student_exams.student_id = ? ORDER BY student_exams.submitted_at DESC LIMIT 5`,
        [studentId],
      ),
    ]);
  return {
    ...
    recentResults: recentRows[0].map((result) => ({
      ...result,
