...
export const updateAuthenticatedUser = async (userId, { name, email, password }) => {
  ...
  try {
    ...
    if (normalizedEmail !== undefined) {
      const [duplicates] = await connection.execute('SELECT id FROM users WHERE email = ? AND id != ? LIMIT 1', [normalizedEmail, userId]);
      if (duplicates[0]) throw new AppError('Email address is already in use', HTTP_STATUS.CONFLICT);
    }
    const fields = []; const values = [];
    if (name !== undefined) { fields.push('name = ?'); values.push(name); }
    if (normalizedEmail !== undefined) { fields.push('email = ?'); values.push(normalizedEmail); }
