...
const translateDuplicateError = (error) => {
  if (error.code === "ER_DUP_ENTRY") {
    throw new AppError(
      "One or more questions are already assigned to this exam",
...
export const assignQuestions = async (examId, questionIds, user) => {
  ...
  try {
    ...
    const totalAssignedQuestions = await getTotalAssignedQuestions(
      connection,
      examId,
    );
