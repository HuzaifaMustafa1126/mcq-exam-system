...
const getTeacherByIdWithExecutor = async (
  executor,
  id,
  { lock = false } = {},
) => {
...
const ensureEmployeeNumberAvailable = async (
  connection,
  employeeNumber,
  ...
  const values = excludedTeacherId
    ? [employeeNumber, excludedTeacherId]
    : [employeeNumber];
  const [rows] = await connection.execute(query, values);
...
const translateDuplicateKeyError = (error) => {
  ...
  if (error.message.includes("uq_users_email"))
    throw duplicateError("email address");
  if (error.message.includes("uq_teachers_employee_number"))
    throw duplicateError("employee number");

  throw new AppError(
    "A teacher with these details already exists",
    HTTP_STATUS.CONFLICT,
  );
};
...
export const createTeacher = async ({
  ...
  try {
    ...
    const teacher = await getTeacherByIdWithExecutor(
      connection,
...
export const updateTeacher = async (id, updates) => {
  ...
  try {
    await connection.beginTransaction();
    const currentTeacher = await getTeacherByIdWithExecutor(connection, id, {
      lock: true,
    });
