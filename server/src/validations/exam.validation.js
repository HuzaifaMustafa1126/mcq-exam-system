...
const subjectIdValidation = body("subjectId")
  .isInt({ min: 1 })
...
const durationValidation = body("durationMinutes")
  .isInt({ min: 1, max: 32767 })
  .withMessage("Duration must be between 1 and 32767 minutes")
  .toInt();
const maxAttemptsValidation = body("maxAttempts")
  .optional()
  .isInt({ min: 1, max: 255 })
  .withMessage("Maximum attempts must be between 1 and 255")
  .toInt();

const totalMarksValidation = body("totalMarks")
  .isFloat({ min: 0, max: 99999999.99 })
  .withMessage("Total marks must be between 0 and 99999999.99")
  .toFloat();

const passingMarksValidation = body("passingMarks")
  .isFloat({ min: 0, max: 99999999.99 })
  .withMessage("Passing marks must be between 0 and 99999999.99")
  .bail()
  .custom(validatePassingMarks)
  .toFloat();
...
const endTimeValidation = body("endTime")
  .optional({ values: "null" })
  .isISO8601()
...
const examIdValidation = param("id")
  .isInt({ min: 1 })
...
export const updateExamValidation = [
  ...
  body().custom((_value, { req }) => {
    ...
    if (
      !Object.keys(req.body).some((field) => editableFields.includes(field))
    ) {
...
export const listExamsValidation = [
  ...
  query("status")
    ...
    .isIn(statuses)
    .withMessage(
      "Status must be draft, published, active, closed, or archived",
    ),
  handleValidationErrors,
];
