...
export default function SubjectsPage() {
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => toast.error(message(error, "Unable to create subject")),
  });
  const updateMutation = useMutation({
    ...
    onError: (error) => toast.error(message(error, "Unable to update subject")),
  });
  const deleteMutation = useMutation({
    ...
    onError: (error) => toast.error(message(error, "Unable to delete subject")),
  });
  const subjects = data?.subjects || [];
  const pagination = data?.pagination;
  const save = (payload) =>
    formSubject?.id
      ? updateMutation.mutate({ id: formSubject.id, payload })
    ...
    <div className="mx-auto max-w-7xl pb-8">
      ...
      <Card className="overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 p-5">
          <div className="relative w-full max-w-md">
            ...
            <input
              ...
              className="w-full rounded-xl border border-white/10 bg-white/[.03] py-2.5 pl-10 pr-4 text-sm text-white outline-none placeholder:text-zinc-500 focus:border-cyan-400/60 focus:ring-4 focus:ring-cyan-400/10"
            />
          </div>
          <div className="flex items-center gap-2">
            <span className="hidden text-sm text-zinc-500 sm:block">
              {pagination ? `${pagination.total} subjects` : "—"}
            </span>
            <ViewButton
              active={view === "card"}
              label="Card view"
        ...
        {isPending ? (
          ...
          <Empty search={deferredSearch} add={() => setFormSubject({})} />
        ) : (
          <>
            {view === "card" ? (
      ...
      <SubjectFormModal
        open={formSubject !== undefined}
        subject={formSubject?.id ? formSubject : null}
...
function SubjectCard({ subject, index, view, edit, remove }) {
  return (
    ...
    >
      ...
      <p className="mt-1 font-mono text-xs text-cyan-200">{subject.code}</p>
...
function SubjectTable({ subjects, view, edit, remove }) {
  return (
        ...
        <tbody>
          {subjects.map((subject, index) => (
            ...
            >
              ...
              <td className="max-w-75 truncate px-5 py-4 text-zinc-400">
                {subject.description || "—"}
              </td>
              <td className="px-5 py-4">
                <Status status={subject.status} />
              </td>
              <td className="px-5 py-4">{subject.totalQuestions}</td>
...
function ViewButton({ children, active, label, onClick }) {
  return (
    <button
      title={label}
      aria-label={label}
      onClick={onClick}
      className={`rounded-lg p-2 ${active ? "bg-cyan-400/15 text-cyan-200" : "text-zinc-500 hover:bg-white/5"}`}
    >
      {children}
    </button>
  );
...
function Status({ status }) {
  return (
    <span
      className={`rounded-full px-2.5 py-1 text-xs font-medium capitalize ${status === "active" ? "bg-emerald-500/10 text-emerald-300" : "bg-amber-500/10 text-amber-300"}`}
    >
      {status}
    </span>
  );
  ...
  return view === "card" ? (
    <div className="grid gap-4 p-5 sm:grid-cols-2 xl:grid-cols-3">
      {Array.from({ length: 6 }, (_, index) => (
        <Skeleton key={index} className="h-62" />
      ))}
    </div>
  ) : (
...
function Error({ retry }) {
  return (
    <div className="p-8 text-center">
      <p className="font-medium text-rose-300">Unable to load subjects.</p>
