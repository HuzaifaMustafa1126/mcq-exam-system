...
import { assignExamQuestions, createExam } from '../services/exams'
...
export default function AdminDashboard() {
  ...
  const mutationOptions = (label) => ({ onSuccess: () => { toast.success(`${label} created successfully`); closeModal(); queryClient.invalidateQueries({ queryKey: ['admin-dashboard'] }) }, onError: (error) => toast.error(error.response?.data?.message || `Unable to create ${label.toLowerCase()}`) })
  const teacherMutation = useMutation({ mutationFn: createTeacher, ...mutationOptions('Teacher') })
  const studentMutation = useMutation({ mutationFn: createStudent, ...mutationOptions('Student') })
  const subjectMutation = useMutation({ mutationFn: createSubject, ...mutationOptions('Subject') })
  const questionMutation = useMutation({ mutationFn: createQuestion, ...mutationOptions('Question') })
  const examMutation = useMutation({ mutationFn: async (payload) => { const { questionIds, ...examPayload } = payload; const exam = await createExam(examPayload); if (questionIds.length) await assignExamQuestions({ id: exam.id, questionIds }); return exam }, ...mutationOptions('Exam') })
  ...
  return <div className="mx-auto max-w-7xl pb-8">
    ...
    <section className="mt-7 grid gap-6 lg:grid-cols-[1fr_320px]">
      ...
      <Card className="p-5"><h2 className="font-bold">Quick actions</h2><p className="mt-1 text-sm text-zinc-500">Manage your examination system.</p><div className="mt-5 grid gap-2 lg:grid-cols-2">{quickActions.map(([label, Icon, action]) => <button type="button" key={action} onClick={() => setOpenModal(action)} aria-label={label} className="flex min-w-0 items-center gap-3 rounded-xl border border-white/5 bg-white/3 px-3 py-3 text-left text-sm text-zinc-300 transition hover:border-cyan-400/25 hover:bg-cyan-400/10 hover:text-white focus:outline-none focus:ring-2 focus:ring-cyan-400"><span className="grid size-8 shrink-0 place-items-center rounded-lg bg-white/5 text-cyan-300"><Icon size={16} /></span><span className="min-w-0 truncate">{label}</span><Plus className="ml-auto shrink-0 text-zinc-500" size={16} /></button>)}</div></Card>
    </section>
    <TeacherFormModal open={openModal === 'teacher'} onClose={closeModal} onSubmit={(payload) => teacherMutation.mutate(payload)} isPending={teacherMutation.isPending} />
    <StudentFormModal open={openModal === 'student'} onClose={closeModal} onSubmit={(payload) => studentMutation.mutate(payload)} isPending={studentMutation.isPending} />
    <SubjectFormModal open={openModal === 'subject'} onClose={closeModal} onSubmit={(payload) => subjectMutation.mutate(payload)} isPending={subjectMutation.isPending} />
    <QuestionFormModal open={openModal === 'question'} onClose={closeModal} onSubmit={(payload) => questionMutation.mutate(payload)} isPending={questionMutation.isPending} />
    <ExamWizardModal open={openModal === 'exam'} exam={null} onClose={closeModal} onSubmit={(payload) => examMutation.mutate(payload)} isPending={examMutation.isPending} />
  </div>
}
...
function ActivityList({ title, items, icon: Icon, getMeta }) {
  return <div><h3 className="mb-3 text-xs font-semibold uppercase tracking-wider text-zinc-500">{title}</h3><div className="space-y-2">{items.length === 0 ? <p className="rounded-xl border border-dashed border-white/10 p-3 text-sm text-zinc-500">No activity yet.</p> : items.map((item, index) => <div key={`${item.name || item.title}-${index}`} className="flex items-center gap-3 rounded-xl bg-white/3 p-3"><span className="grid size-8 shrink-0 place-items-center rounded-lg bg-white/5 text-cyan-300"><Icon size={15} /></span><div className="min-w-0"><p className="truncate text-sm font-medium text-zinc-200">{item.name || item.title}</p><p className="truncate text-xs text-zinc-500">{getMeta(item)}</p></div></div>)}</div></div>
}
