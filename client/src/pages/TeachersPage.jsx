...
export default function TeachersPage() {
  ...
  const queryKey = [
    "teachers",
    { page, limit: PAGE_SIZE, search: deferredSearch },
  ];
  const { data, isPending, isError, refetch } = useQuery({
    queryKey,
    queryFn: () =>
      getTeachers({ page, limit: PAGE_SIZE, search: deferredSearch }),
  });
  const invalidate = () =>
  ...
  const createMutation = useMutation({
    ...
    onError: (error) =>
      toast.error(errorMessage(error, "Unable to create teacher")),
  });
  const updateMutation = useMutation({
    ...
    onError: (error) =>
      toast.error(errorMessage(error, "Unable to update teacher")),
  });
  const deleteMutation = useMutation({
    ...
    onError: (error) =>
      toast.error(errorMessage(error, "Unable to delete teacher")),
  });
  ...
  const saveTeacher = (payload) =>
    formTeacher?.id
      ? updateMutation.mutate({ id: formTeacher.id, payload })
  ...
  return (
    <div className="mx-auto max-w-7xl pb-8">
      <header className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-zinc-400 sm:text-base">
            Create and manage educator accounts for your examination system.
          </p>
        </div>
        <Button onClick={() => setFormTeacher({})}>
          <Plus size={18} />
          Add teacher
        </Button>
      </header>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 p-5">
          <div className="relative w-full max-w-md">
            ...
            <input
              ...
              className="w-full rounded-xl border border-white/10 bg-white/[.03] py-2.5 pl-10 pr-4 text-sm text-white outline-none transition placeholder:text-zinc-500 focus:border-cyan-400/60 focus:ring-4 focus:ring-cyan-400/10"
            />
          </div>
          <p className="text-sm text-zinc-500">
            {pagination ? `${pagination.total} teachers` : "—"}
          </p>
        </div>
        {isPending ? (
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-1">
                          ...
                          <ActionButton
                            label="Edit teacher"
                            onClick={() => setFormTeacher(teacher)}
                          >
                            <Pencil size={16} />
                          </ActionButton>
                          <ActionButton
      ...
      <TeacherFormModal
        open={formTeacher !== undefined}
        teacher={formTeacher?.id ? formTeacher : null}
...
function StatusBadge({ status }) {
  ...
  return (
    <span
      className={`rounded-full px-2.5 py-1 text-xs font-medium capitalize ${colors[status] || "bg-white/10 text-zinc-300"}`}
    >
      {status}
    </span>
  );
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 border-t border-white/10 px-5 py-4">
      ...
      <div className="flex gap-2">
        <button
          ...
          className="rounded-lg border border-white/10 p-2 text-zinc-300 disabled:opacity-40"
        >
          <ChevronLeft size={17} />
        </button>
        <button
...
function ErrorState({ onRetry }) {
  return (
    <div className="p-8 text-center">
      <p className="font-medium text-rose-300">Unable to load teachers.</p>
      <button
        onClick={onRetry}
