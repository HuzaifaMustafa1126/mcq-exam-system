...
export default function QuestionsPage() {
  ...
  const createMutation = useMutation({
    ...
    onError: (error) =>
      toast.error(message(error, "Unable to create question")),
  });
  const updateMutation = useMutation({
    ...
    onError: (error) =>
      toast.error(message(error, "Unable to update question")),
  });
  const deleteMutation = useMutation({
    ...
    onError: (error) =>
      toast.error(message(error, "Unable to delete question")),
  });
              ...
              <table className="w-full min-w-300 text-left text-sm">
                ...
                <tbody>
                  {questions.map((question, index) => (
                    ...
                    >
                      <td className="max-w-110 px-5 py-4">
                        <p className="line-clamp-2 font-medium leading-5 text-white">
                      ...
                      <td className="px-5 py-4">
                        <Badge type="status" value={question.status} />
                      </td>
                      <td className="px-5 py-4 text-zinc-400">
                        {question.createdBy || "Administrator"}
                      </td>
                      <td className="px-5 py-4 text-zinc-400">
                      ...
                      <td className="px-5 py-4">
                        <div className="flex gap-1">
                          ...
                          <Action
                            ...
                            onClick={async () => {
                              try {
                                setFormQuestion(await getQuestion(question.id));
                              } catch (error) {
                                toast.error(
  ...
  const colors =
    type === "difficulty"
      ...
      : {
          ...
          inactive: "bg-white/10 text-zinc-400",
        };
  return (
    <span
      className={`rounded-full px-2.5 py-1 text-xs font-medium capitalize ${colors[value]}`}
    >
      {value}
    </span>
  );
    ...
    <div className="grid min-h-75 place-items-center p-8 text-center">
      <div>
        ...
        <h2 className="mt-4 font-bold">
          {active ? "No questions found" : "No questions yet"}
        </h2>
        <p className="mt-2 text-sm text-zinc-500">
          {active
            ? "Try changing your search or filters."
            : "Create the first question to start building your bank."}
        </p>
        {!active && (
...
function Error({ retry }) {
  return (
    <div className="p-8 text-center">
      <p className="font-medium text-rose-300">Unable to load questions.</p>
