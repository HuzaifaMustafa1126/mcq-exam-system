...
export default function ResultsPage() {
  ...
  const { data: students } = useQuery({
    queryKey: ["students", "result-filter"],
    queryFn: () => getStudents({ page: 1, limit: 100 }),
  });
  const { data: subjects } = useQuery({
    queryKey: ["subjects", "result-filter"],
    ...
    () =>
      Object.values(
        results.reduce((groups, result) => {
          const current = groups[result.subjectName] || {
          ...
          current.count += 1;
          groups[result.subjectName] = current;
          return groups;
        }, {}),
      ).map((item) => ({
        subject: item.subject,
        percentage: Number((item.total / item.count).toFixed(1)),
      })),
    [results],
  );
  const examPerformance = useMemo(
    () =>
      Object.values(
        results.reduce((groups, result) => {
          const current = groups[result.examName] || {
          ...
          current.total += result.percentage;
          current.count += 1;
          groups[result.examName] = current;
          return groups;
        }, {}),
      ).map((item) => ({
        exam: item.exam,
      ...
      <header className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-zinc-400 sm:text-base">
            Analyze performance and review submitted examination results.
          </p>
        </div>
      ...
      {isPending ? (
        ...
        <>
          ...
          <section className="mt-6 grid gap-6 xl:grid-cols-3">
            ...
            <ChartCard title="Exam performance">
              <ResponsiveContainer width="100%" height={230}>
          ...
          <Card className="mt-6 overflow-hidden">
            <div className="space-y-4 border-b border-white/10 p-5">
              <div className="relative max-w-xl">
                ...
                <input
                  ...
                  placeholder="Search student name, student number, or exam name"
                  className="w-full rounded-xl border border-white/10 bg-white/[.03] py-2.5 pl-10 pr-4 text-sm text-white outline-none placeholder:text-zinc-500"
                />
              </div>
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
                ...
                <Select
                  ...
                  onChange={(value) => setFilter("examId", value)}
                  label="All exams"
                >
            ...
            {results.length === 0 ? (
              ...
              <>
                    ...
                    <tbody>
                      {results.map((result, index) => (
                        ...
                        >
                          ...
                          <td className="px-5 py-4">{result.examName}</td>
                          ...
                          <td className="px-5 py-4 text-xs text-zinc-400">
                            {dateTime(result.submittedAt)}
                          </td>
                          <td className="px-5 py-4">
                            <button
                              onClick={() => setDetailsId(result.attemptId)}
                              title="View detailed result"
                              className="rounded-lg p-2 text-zinc-400 hover:bg-white/5 hover:text-cyan-200"
                            >
                              <Eye size={17} />
                            </button>
                          </td>
                ...
                <Pagination data={data.pagination} setPage={setPage} />
              </>
            )}
          </Card>
        </>
...
function Status({ value }) {
  return (
    <span
      className={`rounded-full px-2.5 py-1 text-xs font-medium ${value === "PASS" ? "bg-emerald-500/10 text-emerald-300" : "bg-rose-500/10 text-rose-300"}`}
    >
      {value}
    </span>
  );
  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-4 sm:grid-cols-3">
        {[1, 2, 3].map((id) => (
          <Skeleton key={id} className="h-35" />
        ))}
      </div>
...
function Empty() {
  return (
    <div className="p-12 text-center">
      <XCircle className="mx-auto text-zinc-600" />
...
function Error({ retry }) {
  return (
    <Card className="p-8 text-center">
      <p className="font-medium text-rose-300">Unable to load results.</p>
