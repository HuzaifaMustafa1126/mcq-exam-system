...
const initialFilters = { section: "", status: "" };
...
export default function StudentsPage() {
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => toast.error(message(error, "Unable to create student")),
  });
  const updateMutation = useMutation({
    ...
    onError: (error) => toast.error(message(error, "Unable to update student")),
  });
  const deleteMutation = useMutation({
    ...
    onError: (error) => toast.error(message(error, "Unable to delete student")),
  });
  ...
  const save = (payload) =>
    formStudent?.id
      ? updateMutation.mutate({ id: formStudent.id, payload })
    ...
    <div className="mx-auto max-w-7xl pb-8">
      <header className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-zinc-400 sm:text-base">
            Manage student accounts, class details, and access status.
          </p>
        </div>
      ...
      <Card className="overflow-hidden">
        <div className="space-y-4 border-b border-white/10 p-5">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <p className="text-sm text-zinc-500">
              {pagination ? `${pagination.total} students` : "—"}
            </p>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            <input
              ...
              className="rounded-xl border border-white/10 bg-white/[.03] px-3 py-2.5 text-sm text-white outline-none placeholder:text-zinc-500 focus:border-cyan-400/60"
            />
            <FilterSelect
          ...
          {Object.values(filters).some(Boolean) && (
            <button
              ...
              className="inline-flex items-center gap-1.5 text-sm text-zinc-400 hover:text-cyan-200"
            >
              <FilterX size={15} />
              Clear filters
            </button>
        ...
        {isPending ? (
          ...
          <Empty
            search={deferredSearch || Object.values(filters).some(Boolean)}
            add={() => setFormStudent({})}
          />
        ) : (
          <>
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <div className="flex gap-1">
                          <Action
                            label="View student"
                            onClick={() => setDetailsId(student.id)}
                          >
                            <Eye size={16} />
                          </Action>
                          <Action
      ...
      <StudentFormModal
        ...
        student={formStudent?.id ? formStudent : null}
...
function ErrorState({ retry }) {
  return (
    <div className="p-8 text-center">
      <p className="font-medium text-rose-300">Unable to load students.</p>
