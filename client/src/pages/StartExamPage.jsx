...
export default function StartExamPage() {
  ...
  const exam = useQuery({
    ...
    queryFn: () => getStudentExam(id),
  });
  const start = useMutation({
    mutationFn: () => startStudentExam(id),
    onSuccess: () => navigate(`/exam/${id}/live`),
    onError: (e) => {
      toast.error(e.response?.data?.message || "Unable to start exam");
      if (e.response?.status === 409) navigate(`/exam/${id}/live`);
    },
  });
  if (exam.isLoading)
    return <p className="p-8 text-[#a8b2aa]">Loading exam instructions…</p>;
  if (exam.isError)
    return (
  ...
  return (
    <div className="mx-auto max-w-3xl py-4">
      ...
      <p className="mt-2 text-[#a8b2aa]">{data.subjectName}</p>
      <Card className="mt-7 p-6 md:p-7">
        <div className="grid gap-4 sm:grid-cols-3">
          {[
            [Clock3, `${data.durationMinutes} minutes`],
            [FileText, `${data.totalQuestions} questions`],
            [CheckCircle2, `${data.passingMarks}/${data.totalMarks} to pass`],
          ].map(([Icon, text]) => (
            <div
              key={text}
              className="rounded-xl bg-[#1f5a3a]/20 p-4 text-sm text-[#f5f5f0]"
            >
              <Icon className="mb-2 text-[#c9b86a]" size={19} />
              {text}
            </div>
          ))}
        </div>
        ...
        <Button
          ...
          className="military-button mt-8 w-full"
        >
          {start.isPending ? (
